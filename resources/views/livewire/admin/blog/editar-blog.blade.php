<div>
    <div class="w-full max-w-[1200px] mx-auto py-6 px-4">
        <div class="flex justify-between items-center mb-8">
            <div>
                <h1 class="text-3xl font-bold text-gray-800">Editar Articulo</h1>
                <p class="text-gray-600 mt-2">Modifica el articulo del blog</p>
            </div>
        ...
        <form wire:submit.prevent="actualizar" class="bg-white rounded-lg shadow-lg overflow-hidden">
                ...
                <div class="space-y-6">
                    <div class="bg-gray-50 rounded-lg p-4">
                        ...
                        <div class="mb-4">
                            ...
                            @if($imagen_portada_temp)
                                <div class="mb-3 relative">
                                    <img src="{{ asset('storage/'.$imagen_portada_temp) }}" class="w-full h-40 object-cover rounded-lg border border-gray-200">
                                    <span class="absolute top-2 left-2 bg-green-500 text-white text-xs px-2 py-1 rounded">Actual</span>
                                </div>
                            @endif
                            <input type="file" wire:model="imagen_portada" class="w-full text-sm border border-gray-300 rounded-lg p-2" accept="image/*">
                            <p class="text-xs text-gray-400 mt-1">Selecciona una nueva imagen para reemplazar la actual. Formatos: JPG, PNG, WebP. Max 3MB.</p>
                            @error('imagen_portada') <span class="text-red-500 text-sm">{{ $message }}</span> @enderror
                            @if($imagen_portada)
                                <div wire:loading wire:target="imagen_portada" class="text-blue-500 text-sm mt-1">Subiendo imagen...</div>
                            @endif
                        </div>
                        <div class="mb-4">
            ...
            <div class="flex justify-end space-x-4 px-6 py-4 border-t border-gray-200 bg-gray-50">
                ...
                <button type="submit" class="bg-blue-600 hover:bg-blue-700 text-white px-6 py-3 rounded-lg">Actualizar Articulo</button>
            </div>
        </form>
    </div>
</div>