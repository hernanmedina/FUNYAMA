<div class="w-full" x-data="{
        ...
        scrollLeft() {
            const card = this.scrollContainer.querySelector('.carousel-card');
            if (!card) return;
        ...
        goToSlide(index) {
            ...
            if (!card) return;
            const cardWidth = card.offsetWidth + 24;
            this.scrollContainer.scrollTo({ left: index * cardWidth, behavior: 'smooth' });
        },
        updateActiveSlide() {
            const card = this.scrollContainer.querySelector('.carousel-card');
            if (!card) return;
        ...
        init() {
            ...
            this.scrollContainer.addEventListener('scroll', () => this.updateActiveSlide(), { passive: true });
    ...
    @if($cursos->count() > 0)
        ...
        <div class="relative group">
            ...
            <div class="absolute left-0 top-0 bottom-0 w-10 bg-gradient-to-r from-gray-50 to-transparent z-10 pointer-events-none"
                 x-show="activeIndex > 0" x-transition.opacity></div>

            <!-- Flecha izquierda -->
            <button @click="scrollLeft()" x-show="activeIndex > 0" x-transition
                    class="absolute left-2 top-1/2 -translate-y-1/2 z-20 p-2.5 rounded-full bg-white/90 shadow-lg border border-gray-200 hover:bg-white hover:border-purple-400 hover:shadow-xl transition-all duration-200 opacity-0 group-hover:opacity-100"
                    aria-label="Anterior">
            ...
            <div x-ref="carousel"
                ...
                @foreach($cursos as $curso)
                    <div class="carousel-card snap-start shrink-0 w-[300px] sm:w-[340px] md:w-[370px]">
            ...
            <!-- Flecha derecha -->
            <button @click="scrollRight()" x-show="activeIndex < totalSlides - 1" x-transition
                    class="absolute right-2 top-1/2 -translate-y-1/2 z-20 p-2.5 rounded-full bg-white/90 shadow-lg border border-gray-200 hover:bg-white hover:border-purple-400 hover:shadow-xl transition-all duration-200 opacity-0 group-hover:opacity-100"
            ...
            <div class="absolute right-0 top-0 bottom-0 w-10 bg-gradient-to-l from-gray-50 to-transparent z-10 pointer-events-none"
                 x-show="activeIndex < totalSlides - 1" x-transition.opacity></div>
        </div>

        <!-- Dots indicadores con estado activo -->
        <div class="flex justify-center gap-1.5 mt-6">
            @foreach($cursos as $index => $curso)
                <button @click="goToSlide({{ $index }})"
                        :class="activeIndex === {{ $index }} ? 'bg-purple-600 w-6' : 'bg-gray-300 hover:bg-purple-400'"
