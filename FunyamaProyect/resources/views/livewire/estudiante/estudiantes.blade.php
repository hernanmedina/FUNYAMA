                        ...
                        @foreach($estudiantes as $estudiante)
                            <tr class="hover:bg-gray-50">
                                ...
                                <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-900">
                                    {{ $estudiante->documento_identidad ?? 'No especificado' }}
                                </td>
                                <td class="px-6 py-4 whitespace-nowrap">
                                    <div class="flex items-center space-x-2">
                                        <button wire:click="toggleEstado({{ $estudiante->idEstudiante }})"
                                                class="relative inline-flex h-6 w-11 flex-shrink-0 cursor-pointer rounded-full border-2 border-transparent transition-colors duration-200 ease-in-out focus:outline-none focus:ring-2 focus:ring-blue-500 focus:ring-offset-2
                                                           {{ $estudiante->estado === 'activo' ? 'bg-green-600' : 'bg-gray-200' }}">
                                            <span class="sr-only">Toggle estado</span>
                                            <span aria-hidden="true"
                                                  class="pointer-events-none inline-block h-5 w-5 transform rounded-full bg-white shadow ring-0 transition duration-200 ease-in-out
                                                            {{ $estudiante->estado === 'activo' ? 'translate-x-5' : 'translate-x-0' }}"></span>
                                        </button>
                                        <span class="text-sm font-medium {{ $estudiante->estado === 'activo' ? 'text-green-600' : 'text-gray-500' }}">
                                ...
                                <td class="px-6 py-4 whitespace-nowrap text-sm font-medium">
                                    <div class="flex space-x-2">
                                        <a href="{{ route('estudiantes.show', $estudiante->idEstudiante) }}"
                                        ...
                                        <button wire:click="deleteEstudiante({{ $estudiante->idEstudiante }})"
